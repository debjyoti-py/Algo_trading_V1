...
token_id = token_id
print("="*70)
print("🚀 DHAN MCX TRADING BOT - GOLDPETAL WITH AMO SUPPORT")
print("="*70)
...
# AMO CONFIGURATION - Set to True to enable After-Market Orders
ENABLE_AMO = True
AMO_TIME = 'OPEN'  # Options: 'PRE_OPEN', 'OPEN', 'OPEN_30', 'OPEN_60'
AMO_ENABLED_HOURS = (20, 23)  # 8 PM to 11 PM - only place AMO during these hours
...
print(f"📋 AMO ENABLED: {'✅ YES' if ENABLE_AMO else '❌ NO'}")
if ENABLE_AMO:
    print(f"   AMO Time: {AMO_TIME}")
    print(f"   AMO Hours: {AMO_ENABLED_HOURS[0]}:00 - {AMO_ENABLED_HOURS[1]}:00")
print(f"{'='*70}")
...
def is_market_open():
    """Check if market is currently open (9:30 AM to 5 PM IST)"""
    current_time = datetime.datetime.now()
    market_open = current_time.replace(hour=9, minute=30, second=0, microsecond=0)
    market_close = current_time.replace(hour=17, minute=0, second=0, microsecond=0)
    return market_open <= current_time <= market_close

def should_place_amo():
    """Check if we're in AMO window"""
    if not ENABLE_AMO:
        return False
    current_hour = datetime.datetime.now().hour
    return AMO_ENABLED_HOURS[0] <= current_hour < AMO_ENABLED_HOURS[1]
...
while True:
    ...
    # Check if we should be placing AMO orders
    use_amo = should_place_amo() and is_market_open() == False
    ...
    for scan_name in watchlist:
        ...
        if bc1 and bc2 and bc3 and bc4:
            ...
            # Determine if AMO should be used
            if use_amo:
                print(f"📤 Market closed - Placing AMO order for {trading_symbol}...")
                order_type_str = "AMO"
            else:
                print(f"📤 Market open - Placing Regular order for {trading_symbol}...")
            ...
            try:
                ...
                if margin_available < margin_required and not use_amo:
                ...
                orderbook[trading_symbol]['is_amo'] = use_amo
                orderbook[trading_symbol]['amo_time'] = AMO_TIME if use_amo else None
                
                # PLACE ENTRY ORDER WITH RETRY LOGIC
                entry_orderid = None
                for attempt in range(3):
                    try:
                        print(f"📤 Placing {order_type_str} BUY order for {trading_symbol} (Attempt {attempt+1}/3)...")
                        entry_orderid = tsl.order_placement(
                            ...
                            trade_type='MIS',
                            after_market_order=use_amo,
                            amo_time=AMO_TIME if use_amo else 'OPEN'
                        )
                ...
                # WAIT AND GET EXECUTED PRICE WITH RETRY (only for non-AMO orders)
                entry_price = None
                if use_amo:
                    # For AMO orders, we'll use the current LTP as reference
                    entry_price = trading_ltp
                    print(f"✅ AMO Pending Execution at reference price: ₹{entry_price}")
                else:
                ...
                # PLACE STOP LOSS ORDER WITH RETRY (only for non-AMO orders)
                sl_orderid = None
                if not use_amo:
                    ...
                    print(f"⏸️ Skipping SL for AMO order (will be placed at market open)")
                
                orderbook[trading_symbol]['sl_orderid'] = sl_orderid
                orderbook[trading_symbol]['traded'] = "yes" if not use_amo else "amo_pending"
                ...
                order_entry = {
                    ...
                    'quantity': 1,
                    'is_amo': use_amo,
                    'amo_time': AMO_TIME if use_amo else None,
                    'order_type': order_type_str
                }
                save_order_to_json(order_entry)
                
                message = "\n".join(f"'{key}': {repr(value)}" for key, value in orderbook[trading_symbol].items())
                message = f"{'🌙 AMO' if use_amo else '📈 REGULAR'} ENTRY - {trading_symbol}\n(Signal from {scan_name})\n\n{message}"
                
                print(f"✅ {order_type_str} Order placed!")
        ...
        if orderbook[trading_symbol]['traded'] == "yes":
            try:
                ...
                sl_hit = tsl.get_order_status(orderid=orderbook[trading_symbol]['sl_orderid']) == "TRADED" if orderbook[trading_symbol]['sl_orderid'] else False
                ...
                if sl_hit:
                    ...
                    tsl.send_telegram_alert(message=f"🛑 SL HIT - {trading_symbol}\n💹 PNL: ₹{orderbook[trading_symbol]['pnl']}", receiver_chat_id=receiver_chat_id, bot_token=bot_token)
                    tsl.send_telegram_alert(message=f"🛑 SL HIT - {trading_symbol}\n💹 PNL: ₹{orderbook[trading_symbol]['pnl']}", receiver_chat_id=receiver_chat_id_2, bot_token=bot_token)
                    if reentry == "yes":
                ...
                if tg_hit:
                    ...
                    if orderbook[trading_symbol]['sl_orderid']:
