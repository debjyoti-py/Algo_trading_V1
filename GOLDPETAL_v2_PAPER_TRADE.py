...
- FIXED: Position check before entry ✅
=====================================
...
# Set to track symbols with open positions
open_symbols = set()
...
def update_order_exit(entry_order_id, exit_data, filename='paper_trade.json'):
    ...
    try:
        ...
        if not found:
            ...
            # Fallback: create complete record
...
def update_google_sheets():
    ...
    try:
        if orderbook:
            # Convert orderbook to list of dicts for DataFrame
            positions_list = []
...
# === CHECK FOR EXISTING POSITIONS - DETECT ALL POSITIONS ===
print("\n" + "="*70)
print("🔍 CHECKING FOR EXISTING POSITIONS (ALL POSITIONS)...")
print("="*70)

# FIXED: Load from paper_trade.json (not order_history.json)
try:
    with open('paper_trade.json', 'r') as f:
        orders = json.load(f)

    # Group by entry_order_id to match entry with exit
    order_dict = {}
    for order in orders:
        ...
        if entry_id not in order_dict:
            ...
            # If this record has exit_price and existing doesn't, update it
            if 'exit_price' in order and 'exit_price' not in order_dict[entry_id]:
                order_dict[entry_id] = order

    # Now find truly open positions (no exit_price in final record)
    json_open_positions = []
    ...
    if json_open_positions:
        ...
        for idx, position in enumerate(json_open_positions, 1):
            ...
            # Track symbol as having open positions
        ...
        # Update Google Sheets with all positions
...
"""
    ...
    print("✅ No paper_trade.json file found - starting fresh")
...
# Method 2: Check Dhan account for open positions
try:
...
try:
    while True:
        ...
        for scan_name in watchlist:
            ...
            print(f"   🚫 Blocked Symbols: {', '.join(open_symbols) if open_symbols else 'None'}")
            ...
            try:
                ...
                cc = chart.iloc[-1]
                prev_cc = chart.iloc[-2]  # Previous candle (immediate)

                # BUY signal conditions
                buy_c1 = cc['rsi'] > 65
                buy_c2 = trading_symbol not in open_symbols  # FIXED: Check if symbol is in open_symbols
                ...
                sell_c1 = cc['rsi'] < 35
                sell_c2 = trading_symbol not in open_symbols  # FIXED: Check if symbol is in open_symbols
                ...
                try:
                    ...
                    for attempt in range(3):
                        try:
                            ...
                            # PAPER TRADING - Simulated BUY order
                            entry_orderid = f"PAPER_{current_time.strftime('%Y%m%d_%H%M%S')}_BUY"
                            print(f"📝 PAPER: BUY order {entry_orderid}")
                    ...
                    for attempt in range(5):
                        time.sleep(2)
                        try:
                            # PAPER TRADING - Use LTP
                            entry_price = trading_ltp
                            print(f"📝 PAPER: Entry = ₹{entry_price}")
                    ...
                    orderbook[entry_orderid] = {
                        ...
                        'entry_candle_open': cc['open'],
                        'entry_candle_high': cc['high'],
                        'entry_candle_low': cc['low'],
                        'entry_candle_close': cc['close'],
                        'prev_candle_open': prev_cc['open'],
                        'prev_candle_high': prev_cc['high'],
                        'prev_candle_low': prev_cc['low'],
                        'prev_candle_close': prev_cc['close']
                    }

                    # FIXED: Mark symbol as having open position
                    open_symbols.add(trading_symbol)
                    print(f"🚫 Added {trading_symbol} to blocked symbols. Now blocked: {open_symbols}")
                    ...
                    order_entry = {
                        ...
                        'quantity': 1,
                        'direction': 'BUY',
                        'entry_candle_open': float(cc['open']),
                        'entry_candle_high': float(cc['high']),
                        'entry_candle_low': float(cc['low']),
                        'entry_candle_close': float(cc['close']),
                        'prev_candle_open': float(prev_cc['open']),
                        'prev_candle_high': float(prev_cc['high']),
                        'prev_candle_low': float(prev_cc['low']),
                        'prev_candle_close': float(prev_cc['close']),
                        'entry_rsi': float(cc['rsi']),
                        'entry_macd': float(macd_value),
                        'entry_macd_signal': float(macd_signal),
                        'entry_macd_status': macd_status,
                        'entry_obv': float(obv_value),
                        'entry_obv_ema': float(obv_ema_value),
                        'entry_obv_status': obv_status
                    }

                    save_order_entry(order_entry)

                    # IMPROVED TELEGRAM ALERT - ENTRY
                    message = f"""🟢 BUY ENTRY
📅 Date: {str(current_time.date())}
📊 Symbol: {trading_symbol}
📍 Direction: BUY
...
🟢 Open: ₹{prev_cc['open']:,.2f}
🔺 High: ₹{prev_cc['high']:,.2f}
🔻 Low: ₹{prev_cc['low']:,.2f}
🔴 Close: ₹{prev_cc['close']:,.2f}
...
📊 Total Open Positions: {len(orderbook)}"""
                ...
                print(f"   🚫 BUY signal detected but {trading_symbol} has OPEN POSITION - Entry BLOCKED")
            ...
            # === SELL ENTRY LOGIC ===
                ...
                try:
                    ...
                    for attempt in range(3):
                        try:
                            print(f"📤 Placing SELL order (Attempt {attempt+1}/3)...")

                            # PAPER TRADING - Simulated SELL order
                            entry_orderid = f"PAPER_{current_time.strftime('%Y%m%d_%H%M%S')}_SELL"
                            print(f"📝 PAPER: SELL order {entry_orderid}")
                    ...
                    stop_loss_price = round(prev_cc['high'], 1) # Previous candle high (SL for SELL)
...
📊 Total Open Positions: {len(orderbook)}"""
                ...
                print(f"   🚫 SELL signal detected but {trading_symbol} has OPEN POSITION - Entry BLOCKED")
                ...
                for order_id, position in orderbook.items():
                    try:
                        ...
                        if position['sl_orderid'] is not None:
                            try:
                                # PAPER TRADING - Check LTP vs SL
                                direction_temp = position['buy_sell']
                                if direction_temp == "BUY":
                        ...
                        print(f"   📍 Order {order_id[:15]}... | {direction} | Entry=₹{position['entry_price']} | LTP=₹{ltp} | PNL=₹{current_pnl}")

                        # === STOP LOSS HIT ===
                        if sl_hit:
                            # PAPER TRADING
                            exit_price = position['sl']
                            print(f"📝 PAPER: SL exit = ₹{exit_price}")
...
📊 Remaining Positions: {len(orderbook) - 1}"""
                        ...
                        # === TARGET HIT ===
                            ...
                            if position['sl_orderid'] is not None:
                                try:
                                    # PAPER TRADING - Simulated cancel
                                    print("   📝 PAPER: SL cancelled")
...
📋 Entry Order: {order_id}
📋 Square-off Order: {square_off_order}
📊 Remaining Positions: {len(orderbook) - 1}"""
                ...
                # FIXED: Remove closed positions and update open_symbols
                for order_id in positions_to_remove:
                    if order_id in orderbook:
                        removed_symbol = orderbook[order_id]['name']
                        del orderbook[order_id]

                        # Check if there are any other open positions for this symbol
                        symbol_still_open = any(pos['name'] == removed_symbol for pos in orderbook.values())

                        if not symbol_still_open and removed_symbol in open_symbols:
                            open_symbols.remove(removed_symbol)
                            print(f"✅ Removed {removed_symbol} from blocked symbols - No more open positions")
                            print(f"   Current blocked symbols: {open_symbols if open_symbols else 'None'}")
    ...
    traceback.print_exc()
    try:
        tsl.send_telegram_alert(message=f"🔴 BOT CRASHED\n\nError: {str(e)[:200]}",
                              receiver_chat_id=receiver_chat_id, bot_token=bot_token)
        tsl.send_telegram_alert(message=f"🔴 BOT CRASHED\n\nError: {str(e)[:200]}",
                              receiver_chat_id=receiver_chat_id_2, bot_token=bot_token)
    except:
    ...
    if completed_orders:
        total_pnl = sum([order.get('pnl', 0) for order in completed_orders if order.get('pnl')])
