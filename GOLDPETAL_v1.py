...
print("="*70)
print("🚀 DHAN MCX TRADING BOT - MULTI-POSITION MANAGER (FIXED)")
print("="*70)
...
def save_order_entry(order_data, filename='order_history.json'):
...
def update_order_exit(entry_order_id, exit_data, filename='order_history.json'):
...
# Method 1: Check JSON file for ALL open positions (FIXED LOGIC)
try:
...
📊 Total Open Positions: {len(orderbook)}"""
            ...
            # === SELL ENTRY LOGIC (Similar structure) ===
            ...
            for order_id, position in orderbook.items():
                try:
                    ...
                    if position['sl_orderid'] is not None:
                        try:
                            sl_status = tsl.get_order_status(orderid=position['sl_orderid'])
                    ...
                    📊 Remaining Positions: {len(orderbook) - 1}"""
                            ...
                            print(f"   ❌ Could not place exit order")
