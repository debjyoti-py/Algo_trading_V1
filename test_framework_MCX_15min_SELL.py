...
token_id = token_id
print("="*70)
print("🚀 DHAN MCX TRADING BOT - GOLD FEB Scanner with GOLDPETAL Trading (SELL ONLY)")
print("="*70)
...
TRADING_SYMBOL_OPTIONS = ['GOLDPETAL', 'GOLDPETAL DEC FUT', 'GOLDPETAL31DEC25FUT', 'GOLDPETAL JAN FUT', 'GOLDPETAL31JAN26FUT']
...
while True:
    ...
    for scan_name in watchlist:
        ...
        try:
            ...
            cc = chart.iloc[-2]
            sc1 = cc['rsi'] < 40
            sc2 = orderbook[trading_symbol]['traded'] is None
            sc3 = cc['market_type'] != "neutral"
            sc4 = cc['MACD'] < cc['MACD_Signal']
        ...
        if sc1 and sc2 and sc3 and sc4:
            print(f"🎯 SELL SIGNAL FROM {scan_name}!")
            ...
            try:
                ...
                orderbook[trading_symbol]['tg'] = round(orderbook[trading_symbol]['entry_price']*0.998, 1)
                orderbook[trading_symbol]['sl'] = round(orderbook[trading_symbol]['entry_price']*1.002, 1)
        ...
        if orderbook[trading_symbol]['traded'] == "yes":
            try:
                ...
                if tg_hit:
                    ...
                    time.sleep(2)
                    square_off_order = tsl.order_placement(tradingsymbol=trading_symbol, exchange='MCX', quantity=1, price=0, trigger_price=0, order_type='MARKET', transaction_type='BUY', trade_type='MIS')
                    orderbook[trading_symbol]['exit_time'] = str(current_time.time())[:8]
